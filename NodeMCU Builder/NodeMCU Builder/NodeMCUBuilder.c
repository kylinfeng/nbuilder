...
/******************************************************************************
*
* Local and global Variables for NodeMCUBuilder.c
*
*/
HWND      hwndStatus;
HWND      hwndToolbar;
...
TBBUTTON  tbbMainWnd[] = { {0,IDT_FILE_NEW,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0},
                           ...
                           {3,IDT_FILE_BROWSE,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0},
                           {4,IDT_FILE_DOWNLOAD,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0},
                           {5,IDT_FILE_EXECUTE,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0},
                           {0,0,0,TBSTYLE_SEP,0,0},
                           {6,IDT_EDIT_UNDO,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0},
                           {7,IDT_EDIT_REDO,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0},
                           {0,0,0,TBSTYLE_SEP,0,0},
                           {8,IDT_EDIT_CUT,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0},
                           {9,IDT_EDIT_COPY,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0},
                           {10,IDT_EDIT_PASTE,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0},
                           {0,0,0,TBSTYLE_SEP,0,0},
                           {11,IDT_EDIT_FIND,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0},
                           {12,IDT_EDIT_REPLACE,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0},
                           {0,0,0,TBSTYLE_SEP,0,0},
                           {13,IDT_VIEW_WORDWRAP,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0},
                           {0,0,0,TBSTYLE_SEP,0,0},
                           {14,IDT_VIEW_ZOOMIN,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0},
                           {15,IDT_VIEW_ZOOMOUT,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0},
                           {0,0,0,TBSTYLE_SEP,0,0},
                           {16,IDT_VIEW_SCHEME,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0},
                           {17,IDT_VIEW_SCHEMECONFIG,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0},
                           {0,0,0,TBSTYLE_SEP,0,0},
                           {18,IDT_FILE_EXIT,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0},
                           {19,IDT_FILE_SAVEAS,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0},
                           {20,IDT_FILE_SAVECOPY,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0},
                           {21,IDT_EDIT_CLEAR,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0},
                           {22,IDT_FILE_PRINT,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0},
                           {23,IDT_FILE_OPENFAV,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0},
                           {24,IDT_FILE_ADDTOFAV,TBSTATE_ENABLED,TBSTYLE_BUTTON,0,0} };
...
WCHAR     wchWndClass[16] = WC_NBUILDER;
...
int WINAPI WinMain(HINSTANCE hInstance,HINSTANCE hPrevInst,LPSTR lpCmdLine,int nCmdShow)
{
  ...
  if (!Is2k()) {
    ...
    FormatMessage(
        ...
        0,
        NULL);
    MessageBox(NULL,(LPCWSTR)lpMsgBuf,L"NodeMCU Builder",MB_OK|MB_ICONEXCLAMATION);
    LocalFree(lpMsgBuf);
    return(0);
  }
...
LRESULT CALLBACK MainWndProc(HWND hwnd,UINT umsg,WPARAM wParam,LPARAM lParam)
{
  ...
  switch(umsg)
  {
    ...
    case WM_COPYDATA:
      {
        ...
        if (pcds->dwData == DATA_NBUILDER_PARAMS) {
    ...
    //// This message is posted before NodeMCU Builder reactivates itself
    ...
    case IDM_NODEMCU_BROWSE:
      {
        ...
        if(PathFileExists(tchExeFile))
        {
          ...
          if(lstrlenW(szComName) != 0)
          {
            ...
            lstrcatW(tchParam, szComName);
            lstrcatW(tchParam, L" -list -stay");
            ShellExecuteEx(&sei);
          }
        }
        else
        {
          MsgBox(MBWARN,IDS_ERR_NTERM);
        }
      }
      break;
        ...
        if(PathFileExists(tchExeFile))
        {
          if(lstrlenW(szCurFile) != 0)
          {
            ...
            if(lstrlenW(szComName) != 0)
            {
              ...
              lstrcpyW(tchParam, L"-com:");
              lstrcatW(tchParam, szComName);
              lstrcatW(tchParam, L" -down:\"");
              lstrcatW(tchParam, szCurFile);
              lstrcatW(tchParam, L"\" -stay");
              ShellExecuteEx(&sei);
            }
          }
          else
          {
            SendMessage(hwnd,WM_COMMAND,(WPARAM)MAKELONG(IDM_FILE_OPEN,0),0);
          }
		}
        else
        {
...
void LoadSettings()
{
  ...
  IniSectionGetString(pIniSection,L"OpenComPort",L"",
      szComName,COUNTOF(szComName));
...
void SaveSettings(BOOL bSaveSettingsNow)
{
  ...
  IniSectionSetInt(pIniSection,L"RememberComPort",bRememberCom);
  if(bRememberCom)
  {
    IniSectionSetString(pIniSection,L"OpenComPort",szComName);
  }
  else
  {
...
{
  WCHAR tch[MAX_PATH];
  if (GetPrivateProfileString(L"NodeMCU Builder",L"nbuilder.ini",L"",tch,COUNTOF(tch),lpszFile)) {
...
int FindIniFile() {
  ...
  if (!bFound) {
    lstrcpy(tchTest,L"nbuilder.ini");
    bFound = CheckIniFile(tchTest,tchModule);
  }
...
int TestIniFile() {
  ...
  if (PathIsDirectory(szIniFile) || *CharPrev(szIniFile,StrEnd(szIniFile)) == L'\\') {
    ...
    if (!PathFileExists(szIniFile)) {
      lstrcpy(PathFindFileName(szIniFile),L"nbuilder.ini");
      if (!PathFileExists(szIniFile)) {
...
* ActivatePrevInst()
*
* Tries to find and activate an already open NodeMCU Builder Window
*
*
******************************************************************************/
  ...
  if (flagSingleFileInstance && lpFileArg) {
    ...
    if (hwnd != NULL)
    {
      ...
      if (IsWindowEnabled(hwnd))
      {
        ...
        cds.dwData = DATA_NBUILDER_PARAMS;
...
//  SnapToDefaultPos()
//
//  Aligns NodeMCU Builder to the default window position on the current screen
//
//
...
void ShowNotifyIcon(HWND hwnd,BOOL bAdd)
{
  ...
  nid.hIcon = hIcon;
  lstrcpy(nid.szTip,L"NodeMCU Builder");
