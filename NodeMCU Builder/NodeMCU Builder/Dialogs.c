...
*
* Dialogs.c
*   NodeMCU Builder dialog boxes implementation
*
          ...
          case NM_RETURN:
            {
              if (pnmhdr->idFrom == IDC_WEBPAGE) {
                ShellExecute(hwnd,L"open",L"http://www.cnblogs.com/youlin/",NULL,NULL,SW_SHOWNORMAL);
              }
              else if (pnmhdr->idFrom == IDC_EMAIL) {
                ShellExecute(hwnd,L"open",L"mailto:fengyoulin@foxmail.com",NULL,NULL,SW_SHOWNORMAL);
              }
            }
            break;
        }
        ...
        case IDOK:
          {
            ...
            if (GetDlgItemText(hwnd,IDC_COMMANDLINE,arg1,MAX_PATH))
            {
              ...
              if (lstrcmpi(arg1,L"nbuilder") == 0 ||
                  lstrcmpi(arg1,L"nbuilder.exe") == 0) {
...
//  SelectDefLineEndingDlgProc()
//
//  Controls: IDC_COMPORTLIST
//            IDC_REMEMBERMYCHOICE
//
void GetCommPortList(HWND hwnd)
{
	HKEY hKey;
	if(ERROR_SUCCESS == RegOpenKeyEx(HKEY_LOCAL_MACHINE, L"Hardware\\DeviceMap\\SerialComm", 0, KEY_READ, &hKey))
	{
		...
		while(TRUE)
		{
			...
			if(ERROR_NO_MORE_ITEMS == RegEnumValue(hKey, i, RegKeyName, &dwLong, NULL, NULL, (PUCHAR)SerialPortName, &dwSize))
			{
				break;
			}
			if(!memcmp(SerialPortName, L"COM", 6))
			{
				SendDlgItemMessage(hwnd,IDC_COMPORTLIST,CB_ADDSTRING,0,(LPARAM)SerialPortName);
			}
			++i;   
		}
		RegCloseKey(hKey);
...
INT_PTR CALLBACK SelectComPortDlgProc(HWND hwnd,UINT umsg,WPARAM wParam,LPARAM lParam)
{
	switch(umsg)
	{
	case WM_INITDIALOG:
		{
			GetCommPortList(hwnd);

			// Load options
			SendDlgItemMessage(hwnd, IDC_COMPORTLIST, CB_SELECTSTRING, 0, (LPARAM)szComName);

			if (bRememberCom)
				CheckDlgButton(hwnd,IDC_REMEMBERMYCHOICE,BST_CHECKED);
	...
	case WM_COMMAND:
		...
		case IDOK:
			{
				int curSel = SendDlgItemMessage(hwnd,IDC_COMPORTLIST,CB_GETCURSEL,0,0);
				if(curSel >= 0 && SendDlgItemMessage(hwnd,IDC_COMPORTLIST,CB_GETLBTEXTLEN,(WPARAM)curSel,0) < MAX_PATH)
				{
					SendDlgItemMessage(hwnd,IDC_COMPORTLIST,CB_GETLBTEXT,(WPARAM)curSel,(LPARAM)szComName);
				}
				bRememberCom = (IsDlgButtonChecked(hwnd,IDC_REMEMBERMYCHOICE) == BST_CHECKED) ? 1 : 0;
				EndDialog(hwnd,IDOK);
			}
			break;
...
{
	INT_PTR iResult = ThemedDialogBoxParam(g_hInstance,MAKEINTRESOURCE(IDD_COMSELECT),
		hwnd,SelectComPortDlgProc,(LPARAM)lpstrDefault);
	if(iResult == IDOK)
	{
		return TRUE;
	}
...
INT_PTR CALLBACK BrowseFileDlgProc(HWND hwnd,UINT umsg,WPARAM wParam,LPARAM lParam)
{
	switch(umsg)
	{
	case WM_INITDIALOG:
		{
			...
			// Add columns
			SendDlgItemMessage(hwnd, IDC_BROWSEFILELIST, LVM_INSERTCOLUMN, 0, (LPARAM)&lvColumn);

			lvColumn.cx = 80;
			lvColumn.pszText = L"Size";

			SendDlgItemMessage(hwnd, IDC_BROWSEFILELIST, LVM_INSERTCOLUMN, 1, (LPARAM)&lvColumn);

			// Add items
			lvItem.mask = LVIF_TEXT;
			lvItem.pszText = L"init.lua";
			lvItem.iItem = 0;
			lvItem.iSubItem = 0;

			SendDlgItemMessage(hwnd, IDC_BROWSEFILELIST, LVM_INSERTITEM, 0, (LPARAM)&lvItem);

			lvItem.pszText = L"3323";
			lvItem.iSubItem = 1;

			SendDlgItemMessage(hwnd, IDC_BROWSEFILELIST, LVM_SETITEM, 0, (LPARAM)&lvItem);
...
{
	INT_PTR iResult = ThemedDialogBoxParam(g_hInstance,MAKEINTRESOURCE(IDD_FILEBROWSE),
		hwnd,BrowseFileDlgProc,(LPARAM)lpstrDefault);
	if(iResult == IDOK)
	{
		return TRUE;
	}
